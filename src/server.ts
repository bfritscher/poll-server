...
app.get('/primus/primus.js', (_req: express.Request, res: express.Response) => {
  ...
  spark.on('data', (data) => {
    ...
    if (action === 'vote' && !user.isAdmin) {
      ...
      if (question && !question.stop) {
        ...
        primus
          .room(roomName)
          .write({a: 'votesCount', q: data.q, v: question.votesCount()});
        primus.room(roomAdminName).write({
          a: 'vote',
          u: user.email,
          v: data.v,
    ...
    if (action === 'add_question') {
      ...
      primus
        .room(roomName)
        .write({a: 'questionsCount', v: room.questions.length});
      return;
    }
    ...
    if (action === 'set_state') {
      ...
      if (
        question &&
        !question.stop &&
        (Object.keys(question.votes).length > 0 || data.v === room.state)
      ) {
      ...
      if (data.v === 'results') {
        ...
        primus
          .room(roomName)
          .write({a: 'state', v: 'results', results: room.results()});
      }
      return;
    }
